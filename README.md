...
3. In the Runtime API service's **Variables** tab, add `PSEUDONYM_TENANT_SECRET` with the output of `openssl rand -hex 32`. The default CORS allow-list contains the local consumer and `https://lorb-production-consumer.up.railway.app`. Optionally set `ALLOWED_CONSUMER_ORIGINS` to replace those defaults with a comma-separated list of exact origins (no paths, trailing slashes, or wildcards). Do not add `PORT`; Railway injects it for the service.
