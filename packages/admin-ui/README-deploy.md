# Deploying the Administration Workspace
...
| `VITE_ADMIN_API_BASE` | The administration prefix, e.g. `https://runtime.lorb.example/api/v1/admin` |
| `VITE_RUNTIME_API_BASE` | The Runtime prefix, for the catalogue and smart-link pages |
...
The workspace additionally needs the Runtime API to be running with `ADMIN_ALLOWED_ROLES` and
`ADMIN_APPROVAL_REQUIRED_FOR` configured, and your provider emitting the role claim those expect.
Without the claim, every route here answers 403 — which is the correct answer for a token that does
not carry it.
