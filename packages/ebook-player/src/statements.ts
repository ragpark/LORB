/**
 * xAPI statement construction for the generic EPUB 3 ebook reader. Same verb-reuse reasoning as
 * document-player, whose statements these mirror one for one: launched / answered(quartile of the
 * spine read) / completed, no new verbs added to the Evidence API's accepted set.
 */
...
/** Furthest-spine-item-reached quartile, e.g. quartile = 50 -> response "p50". */
...
/** Which quartile checkpoints crossed as the furthest page reached moves from `previousPage` to
 * `page` (both zero-indexed) out of `totalPages`. */
export function quartilesCrossed(previousPage: number, page: number, totalPages: number): Array<25 | 50 | 75> {
  if (totalPages <= 1) return [];
  const previousFraction = previousPage / (totalPages - 1);
  const fraction = page / (totalPages - 1);
