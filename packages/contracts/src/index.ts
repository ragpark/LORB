...
export const descriptorSchema = z.object({iss:z.string().url(),aud:z.literal("lorb-player"),iat:z.number().int(),nbf:z.number().int(),exp:z.number().int(),jti:uuid,sub:z.string().regex(/^[a-f\d]{64}$/),tenant_id:z.literal("mvp-tenant"),repository_id:uuid,consumer_id:z.string().min(1),object_id:uuid,object_version_id:uuid,package_version_id:uuid,delivery_profile:z.literal("native-web-package"),launch_mode:z.literal("embedded-iframe"),player_ref:z.string().regex(/^mvp-shell-v\d+$/),correlation_id:uuid,locale:z.literal("en-GB"),attempt_id:uuid,state_endpoint:z.string().url(),evidence_endpoint:z.string().url(),package_url:z.string().url(),session_config:z.object({expires_at:z.string().datetime()}),telemetry_config:z.object({correlation_header:z.literal("X-Correlation-ID")}),contract_version:z.literal("1.0")}).strict().refine((v: {exp:number;iat:number})=>v.exp-v.iat===600,"descriptor lifetime must be 600 seconds");
