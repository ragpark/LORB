import {z} from 'zod';
import {ENVIRONMENT_LABELS,readAuthConfig,type AuthClientConfig,type EnvironmentLabel} from '@lorb/web-auth';
...
export interface Config {runtimeApiBase:string;adminApiBase:string;runtimeIssuer:string;jwksUrl:string;playerShellOrigin:string;
 ...
 /** The configured sign-in — a provider, or a gateway session — where there is one. Its absence is what enables the development path. */
...
export function readConfig(env:ImportMetaEnv=webEnv):Config{
 ...
 const auth=readAuthConfig(env,()=>location.origin);
