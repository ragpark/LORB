...
export interface Config {runtimeApiBase:string;runtimeIssuer:string;jwksUrl:string;playerShellOrigin:string;stubLoginUrl:string;stubIssuer:string;environment:Environment;allowedShellOrigins:ReadonlySet<string>}
export function readConfig(env:ImportMetaEnv=import.meta.env):Config{
 ...
 return {runtimeApiBase,runtimeIssuer,jwksUrl:env.VITE_JWKS_URL??'http://localhost:3000/api/v1/runtime/jwks',playerShellOrigin,stubLoginUrl:env.VITE_STUB_IES_LOGIN_URL??'http://localhost:4000/dev-login',stubIssuer:env.VITE_STUB_IES_ISSUER??'http://localhost:4000',environment,allowedShellOrigins:new Set(origins)};
}
