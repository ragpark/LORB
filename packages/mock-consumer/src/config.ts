...
export function readConfig(env:ImportMetaEnv=import.meta.env):Config{
 ...
 return {runtimeApiBase:env.VITE_RUNTIME_API_BASE??'http://localhost:3000/api/v1/runtime',runtimeIssuer:env.VITE_RUNTIME_ISSUER??'http://localhost:3000',jwksUrl:env.VITE_JWKS_URL??'http://localhost:3000/api/v1/runtime/jwks',playerShellOrigin,stubLoginUrl:env.VITE_STUB_IES_LOGIN_URL??'http://localhost:4000/dev-login',stubIssuer:env.VITE_STUB_IES_ISSUER??'http://localhost:4000',environment,allowedShellOrigins:new Set(origins)};
}
