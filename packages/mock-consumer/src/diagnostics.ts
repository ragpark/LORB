...
export const redactedHeaders=(headers:HeadersInit)=>{const entries=headers instanceof Headers?[...headers.entries()]:Array.isArray(headers)?headers:Object.entries(headers);return Object.fromEntries(entries.map(([k,v])=>[k,k.toLowerCase()==='authorization'?'Bearer ...redacted...':v]))};
