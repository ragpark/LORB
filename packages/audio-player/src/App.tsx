...
import type { AudioContent, LaunchContext } from "../../contracts/src/index.js";
...
type DeliveredContent = AudioContent & { launch_context?: LaunchContext };
...
export function App() {
  ...
  useEffect(() => {
    ...
    void fetch(context.content_url, { headers: { accept: "application/json" } })
      ...
      .catch((error: Error) => {
        ...
        port.postMessage(envelope("experience.error", { code: "AUDIO_CONTENT_UNAVAILABLE", recoverable: false, detail: error.message }, context.correlation_id));
      });
  ...
  const onTimeUpdate = useCallback((event: React.SyntheticEvent<HTMLAudioElement>) => {
    if (!context) return;
  ...
  return (
    <main style={styles.main}>
      <h1 style={styles.h1}>{content?.title ?? "Audio"}</h1>
      {content?.description && <p style={styles.lede}>{content.description}</p>}

      {phase === "waiting" && (
        <p style={styles.notice} role="status">
          Waiting for activity context from the Player Shell… This audio must run inside an embedded LORB launch.
        </p>
      )}
      {phase === "loading" && <p style={styles.notice} role="status">Loading audio…</p>}
        ...
        <section style={styles.card}>
          ...
          {phase !== "completed" && (
            <div style={styles.actions}>
              <button type="button" style={styles.secondary} onClick={complete}>Mark as listened</button>
            </div>
          )}
        </section>
      )}
