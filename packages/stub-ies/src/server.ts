...
app.post("/dev-login",async(req:any,reply)=>{const subject=req.body?.subject;if(typeof subject!=="string"||!/^synthetic-[a-z0-9-]+$/.test(subject))return reply.code(400).send({code:"SYNTHETIC_IDENTITY_REQUIRED"});return {access_token:await issueIesToken(privateKey,subject,"lorb-runtime",issuer),token_type:"Bearer",expires_in:600}});
