...
export async function issueDescriptor(privateKey:KeyLike, claims:Record<string,unknown>){
 const now=Math.floor(Date.now()/1000); const payload={...claims,iss:"http://localhost:3000",aud:"lorb-player",iat:now,nbf:now,exp:now+600,jti:randomUUID(),tenant_id:"mvp-tenant",delivery_profile:"native-web-package",launch_mode:"embedded-iframe",player_ref:"mvp-shell-v1",evidence_endpoint:"http://localhost:3100/api/v1/evidence/statements",telemetry_config:{correlation_header:"X-Correlation-ID"},contract_version:"1.0"}; descriptorSchema.parse(payload);
...
export async function verifyDescriptor(token:string,key:KeyLike){const {payload}=await jwtVerify(token,key,{issuer:"http://localhost:3000",audience:"lorb-player",algorithms:["ES256"]});return descriptorSchema.parse(payload);}
